...
def init_prior_pos_proba(
    ...
    ''' Set prior proba probabilities using 
        ...
        save_to: string
                 path to save the resulting dictionary
    ...
    tag_counts = dict()
    if lexicon is None:
        lexicon = nltk.corpus.brown.tagged_words()
    ...
    for word, tag in data:
        ...
        tag_idx = np.where(tags_list == tag)[0][0]
        tag_counts[word][tag_idx] += 1

    if not save_to is None:
        cPickle.dump(open(save_to, 'wb'), tag_counts)
...
def lookup_prior_pos_proba(tokens, tag_counts=None):
    ...
    if tag_counts is None:
        tag_counts = cPickle.load(
            open('storage/tag_counts.pkl', 'rb'))
